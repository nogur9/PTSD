...
X_train_0, X_test_0, y_train_0, y_test_0 = train_test_split(X, Y, test_size=0.25,
                                                            random_state=271828, stratify=df['PCL_Strict3'].values)
X_train, X_test, y_train, y_test = train_test_split(X_train_0, y_train_0, test_size=0.2)
...
params_bart = {
    ...
    'classifier__sigma_a': [0.1, 0.01, 0.5],
    'classifier__sigma_b': [0.1, 0.01, 0.5],
    'classifier__n_samples': [100, 50, 200],

}
loo = StratifiedKFold(3)
                ...
                for n_samples in [100, 50, 200]:
                    for sigma_b in [0.0001, 0.01, 0.001]:
                        for sigma_a in [0.0001, 0.01, 0.001]:
                            gs_xgb = SklearnModel(sigma_a=sigma_a, sigma_b=sigma_b, n_samples=n_samples).fit(X_train, y_train)
                            y_pred = gs_xgb.predict(X_test)
                            # Test data accuracy of model with best params
                            #print('Test set score score for best params: %.3f ' % mean_squared_error(y_test, y_pred))
                            print('Test set score score for best params: %.3f ' % f1_score(y_test, y_pred > cutoff))
