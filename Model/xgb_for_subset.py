...
target_features_2 = ["tred_cutoff"]

ID = ["ID"]
for drop in [0, 30, 45]:
    for trg in target_features_2 + target_features:
        for PCL_Strict1 in [-1]:
            for phq9 in [3, -1, 1, 5]:
                for highschool in [0, -1]:
                    for exp in [1, 0]:
                        ...
                        try:
                            ...
                            df = df.dropna(thresh=drop, axis=0)

                            mice = IterativeImputer()
                            df = pd.DataFrame(mice.fit_transform(df), columns=df.columns)
                            #PCL_Strict1
                            df = df[df["PCL_Strict1"] > PCL_Strict1]

                            # phq9
                            df = df[df["phq1"] > phq9]
                            # high school
                            df = df[df["highschool_diploma"] > highschool]
                            ...
                            print(f"c = {c}")
                            X_train, X_test, y_train, y_test = train_test_split(X_train_0, y_train_0, test_size=0.15, stratify=y_train_0)
                            ...
                            params_xgb = {
                                'classifier__n_estimators': [100, 300, 50],
